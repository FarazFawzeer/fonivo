@extends('layouts.vertical', ['subtitle' => 'Supplier Ledger'])

@section('content')
    @include('layouts.partials.page-title', ['title' => 'Ledgers', 'subtitle' => 'Supplier'])

    <div class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">Supplier Ledger</h5>
            <p class="card-subtitle mb-0">Purchases (Debit) vs Payments (Credit) with running balance.</p>
        </div>

        <div class="card-body">
            <form method="GET" action="{{ route('admin.ledgers.suppliers.index') }}" class="row g-2 mb-3">
                <div class="col-md-4">
                ...
                <div class="col-md-2 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.ledgers.suppliers.index') }}" class="btn btn-light w-100">Reset</a>
                </div>
            </form>

            @if($supplier)
                <div class="row mb-3">
                    <div class="col-md-4"><b>Supplier:</b> {{ $supplier->name }}</div>
                    <div class="col-md-3"><b>Purchases (Debit):</b> {{ number_format((float)($summary['total_debit'] ?? 0),2) }}</div>
                            ...
                            @forelse($rows as $r)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($r['type'] === 'purchase')
                                            <a href="{{ route('admin.purchases.show', $r['related_id']) }}"
            ...
            @else
                <div class="alert alert-info mb-0">Select a supplier to view ledger.</div>
            @endif
        </div>
    </div>
@endsection
