@extends('layouts.vertical', ['subtitle' => 'Sales Details'])

@section('content')
    @include('layouts.partials.page-title', ['title' => 'Sales', 'subtitle' => 'Details'])
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                <h5 class="card-title mb-0">{{ $sale->invoice_no }}</h5>
        ...
        <div class="card-body">
            ...
            <div class="row mb-3">
                <div class="col-md-3"><b>Total:</b> {{ number_format($total,2) }}</div>
                ...
                <table class="table table-hover table-centered">
                    ...
                    <tbody>
                        @foreach($sale->items as $it)
                            <tr>
                                <td>{{ $it->phone_unit_id ? 'Phone' : 'Accessory' }}</td>
                                <td>
                                    <b>{{ $it->product?->name ?? '-' }}</b>
            ...
            <div id="payBox" style="display:none;" class="border rounded p-3 mb-3">
                ...
                <form id="payForm" action="{{ route('admin.sales.payments.store', $sale->id) }}" method="POST">
                    @csrf
    ...
    <script>
        ...
        document.getElementById('payForm').addEventListener('submit', function(e){
            ...
            .catch(err => {
                document.getElementById('payMessage').innerHTML = `<div class="alert alert-danger">Error: ${err}</div>`;
            });
        });
    </script>
@endsection
